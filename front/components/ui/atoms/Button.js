...
function Button({ label, status, onClick,className }) {
  return (
    <>
      {/* <button className={` text-white flex items-center  justify-center font-normal  transition-default hover:opacity-75 ${className}`}>
        {label}
      </button> */}
      ...
      <button onClick={onClick} className={clsx(
        " flex items-center justify-center font-normal transition-default hover:opacity-75",
        {
          ...
          'md:w-full h-[40px] rounded-md text-[1.6rem]  bg-white border-2 border-primary text-primary  ': status === 'cancelBtn',
        }
      )}>{label}</button></>
